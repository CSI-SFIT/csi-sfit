...
const toMapsLink = (location: string, explicit?: string) =>
  explicit || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(location)}`;
...
type SpeakerLinks = Record<string, { instagram?: string; github?: string; linkedin?: string }>;
...
export const Events: React.FC = () => {
  ...
  const categories = useMemo(
    () => [
      ...
      { id: 'hackathon', label: 'Hackathons', count: events.filter((e) => e.category === 'event').length },
  ...
  const getStatusColor = (status: string) => {
    ...
    return (colors as any)[status] || 'bg-[#2580E4]/20 text-[#2580E4] border-[#2580E4]/30';
  };
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-6">
              Events &{' '}
              <span className="bg-gradient-to-r from-[#40E0D0] to-[#1A5AFF] bg-clip-text text-transparent">Workshops</span>
            </h1>
            <p className="text-xl text-gray-400 max-w-3xl mx-auto leading-relaxed">
              Join our exciting events, workshops, and competitions designed to enhance your technical skills and connect with
              the tech community.
            </p>
          </motion.div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {filteredEvents.map((event, index) => {
                const speakerLinks = (event as unknown as { speakerLinks?: SpeakerLinks }).speakerLinks;
                const explicitMaps = (event as unknown as { locationLink?: string }).locationLink;
                  ...
                  >
                    <GlassCard className="group cursor-pointer transition-all duration-300 transform hover:scale-[1.02] hover:shadow-[0_8px_24px_rgba(54,183,183,0.3)]">
                      ...
                      <div className="p-6">
                        ...
                        <p className="text-gray-400 text-sm mb-4 whitespace-pre-line">{event.description}</p>

                        <div className="space-y-2 mb-4">
                          <button
                            type="button"
                            onClick={() => window.open(toCalendarLink(event), '_blank', 'noopener,noreferrer')}
                          ...
                          <button
                            type="button"
                            onClick={() => window.open(toMapsLink(event.location, explicitMaps), '_blank', 'noopener,noreferrer')}
                            className="flex items-center text-left text-sm text-gray-300 hover:text-white transition cursor-pointer underline decoration-dotted underline-offset-4"
                          ...
                          {event.entry && (
                            <div className="flex items-center text-sm text-gray-300">
                              ...
                              <span className="font-medium">Entry:</span>&nbsp;{event.entry}
                            </div>
                          )}
                            ...
                            <div className="pt-2">
                              ...
                              <div className="flex flex-col gap-2">
                                {event.speakers.map((name, i) => {
                                  const links = (speakerLinks as SpeakerLinks | undefined)?.[name] || {};
                                  const hasAny = links.instagram || links.github || links.linkedin;
                        ...
                        <motion.button
                          ...
                          onClick={() => {
                            if (event.formLink) window.open(event.formLink, '_blank', 'noopener,noreferrer');
                          }}
                        ...
                        >
                          ...
                          {event.formLink && <ExternalLink className="w-4 h-4" />}
                        </motion.button>
                      </div>
                    </GlassCard>
                  </motion.div>
          ...
          {filteredEvents.length === 0 && (
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-center py-20">
              <GlassCard className="p-12 max-w-md mx-auto">
                ...
                <p className="text-gray-400">No events match your current search criteria. Try adjusting your filters.</p>
                <motion.button
